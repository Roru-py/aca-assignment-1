    ...
    //ezsum same as that in simd.cpp
    ...
    //all simd parts are same as that in simd.cpp so taken from there
    void matmul_prefetch(const float* A, const float* B, float* C,
        ...
        for(long j0=0;j0<N;j0+=tile_len) {
            ...
            //below same as simd.cpp except for pefetch instructions
            long i=0;
            for(;i<=M-6;i+=6) {
                long j=j0;
                for(;j<=jend-2;j+=2) {
                    ...
                    long k=0;
                    long limit=(K>pf_dist)?(K-pf_dist):0;
                    for(;k<=limit-8;k+=8) {
